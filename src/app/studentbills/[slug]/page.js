...
function Page() {
  ...
  const handleMakePayment = async () => {
    ...
    let res = await createPayment(stringifyObject({ payment, type: "student",user }));
    res = parseString(res);
    if (res.status === 200) {
  ...
  const handleDeletePayment = async(paymentId) => {
    let res = await deletePayment(stringifyObject({paymentId,type:"student"}));
    res = parseString(res);
    if (res.status === 200) {
  ...
  const handleBillServiceDelete = async (index) => {
    ...
    let res = await deletePurchase(
      stringifyObject({ billId: slug, purchaseId: serviceId, type: "student" })
    );
    res = parseString(res);
    if (res.status === 200) {
  ...
  const handleSaveProduct = async (index) => {
    ...
    let res = await axios.post('/api/purchases/update/student', data, {
      headers: {
  ...
  const getAllPayments = async () => {
    let res = await getPaymentsByBillId(
      stringifyObject({ billId: slug, type: "student" })
    );
    res = parseString(res);
    if (res.status === 200) {
  ...
  return (
    <div>
      <Header />
      <br />
      {bill && <h1>Student Bill #{bill._id}</h1>}
      <div>Date: {bill&& 
